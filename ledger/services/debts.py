...
def debt_rows(workspace=None):
    money = DecimalField(max_digits=20, decimal_places=2)
    rows = PartnerBalance.objects.select_related('counterparty', 'batch')
...
def record_payment(debt_id, *, amount, date, note, request_key, revision, actor, workspace=None):
    initial = PartnerBalance.objects.get(pk=debt_id, **({'workspace': workspace} if workspace is not None else {}))
    with workspace_transaction(initial.workspace):
        previous = DebtPayment.objects.filter(request_key=request_key, debt__workspace=initial.workspace).first()
        if previous:
        ...
        debt = PartnerBalance.objects.get(pk=debt_id, workspace=initial.workspace)
...
def cancel_payment(payment_id, *, reason, revision, actor, workspace=None):
    initial = DebtPayment.objects.select_related('debt').get(pk=payment_id, **({'debt__workspace': workspace} if workspace is not None else {}))
    with workspace_transaction(initial.debt.workspace):
        payment = DebtPayment.objects.select_related('debt').get(pk=payment_id, debt__workspace=initial.debt.workspace)
